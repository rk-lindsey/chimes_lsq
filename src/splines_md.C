...
int main(int argc, char* argv[])
{
	...
	if ( RANK == 0 ) cout << "   ...read complete" << endl << endl;	
 ...
 FF_SETUP_1: 
	...
	if ( RANK == 0 ) cout << "Reading atom info from parameter file..." << endl; 
	...
	while (FOUND_END == false)
		{
			...
			if(LINE.find("# PAIRIDX #") != string::npos)
			...
			else if(LINE.find("ATOM TYPES:") != string::npos)
				{
					...
					if ( RANK == 0 ) cout << "	Read " << NATMTYP << " atom types:" << endl;			
				}	
	...
	if ( RANK == 0 ) cout << "Setting up thermostats/velocities..." << endl; 
	...
	if ( RANK == 0 ) cout << "   ...setup complete" << endl << endl;
	...
	if ( RANK == 0 ) cout << "Running velocity sanity checks..." << endl; 
 ...
 FF_SETUP_2:
	...
	while (FOUND_END == false)
		{
			...
			// Read in pair potential info
				...
				{	
							...
							for(int i=0; i<NO_PAIRS; i++) 
								{
									...
									// Reduce penalty_scale for better energy conservation (LEF)
					...
					if ( RANK == 0 ) cout << "	...Read general FF params..." << endl;
				}
			...
			// Setup triplets
				...
				{	
					...
					if ( RANK == 0 ) cout << "	...Read FF triplet specifications..." << endl;
			...
			// Read in pair parameters
				...
				{	
					...
					if ( RANK == 0 ) cout << "	...Reading all remaining force field parameters..." << endl;
					...
					if ( RANK == 0 ) cout << "	...Read 2-body FF params..." << endl;
				}		
			...
			// Read 3B parameters
				...
				{	
					...
					if ( RANK == 0 ) cout << "	...Read 3-body FF params..." << endl << endl;;				
				}
			...
			// Read in the fit overbonding parameter
			...
			else if(LINE.find("P OVER: ") != string::npos)
				{
					...
					if ( RANK == 0 ) cout << "	...Read ReaxFF overbonding FF params..." << endl;
				}
			...
			// Read the pair maps
			...
			else if(LINE.find("PAIRMAPS: ") != string::npos)
				{
					...
					if ( RANK == 0 ) cout << "	Reading  " << TMP_TERMS1 << " pairs for mapping" << endl;
					...
					for(int i=0; i<TMP_TERMS1; i++)
						{
							...
							if ( RANK == 0 ) cout << "	........Reading pair: " << TEMP_TYPE << " with mapped index: " << TMP_TERMS2 << endl; 
					...
					if ( RANK == 0 ) cout << "	...Read FF pairmaps..." << endl << endl;					
				}	
			...
			// Read the pair maps
			...
			else if(LINE.find("TRIPMAPS: ") != string::npos)
				{
					...
					if ( RANK == 0 ) cout << "	Reading  " << TMP_TERMS1 << " triplets for mapping" << endl;
					...
					for(int i=0; i<TMP_TERMS1; i++)
						{
							...
							if ( RANK == 0 ) cout << "	........Reading triplet: " << TEMP_TYPE << " with mapped index: " << TMP_TERMS2 << endl; 
					...
					if ( RANK == 0 ) cout << "	...Read FF triadmaps..." << endl;					
				}						
	...
	for(CONTROLS.STEP=0;CONTROLS.STEP<CONTROLS.N_MD_STEPS;CONTROLS.STEP++)	//start Big Loop here.
		{
			...
			// Synchronize forces, energy, and pressure.
#ifdef USE_MPI
	...
	if ( RANK == 0 ) cout << "END SIMULATION" << endl;
...
static void read_input(MD_JOB_CONTROL & CONTROLS, PES_PLOTS & FF_PLOTS) 				// UPDATED
{
	if ( RANK == 0 ) cout << endl << "Reading the simulation control input file..." << endl;
	...
	while (FOUND_END == false)
		{
			...
			// "General control variables"
			...
			else if(LINE.find("# CRDFILE #") != string::npos)
				{
					...
					if ( RANK == 0 ) cout << "	# CRDFILE #: " << CONTROLS.COORD_FILE << endl;	
				}			
			...
			// " Simulation options"
			...
			else if(LINE.find("# VELINIT #") != string::npos)
				{
					...
					if(CONTROLS.INIT_VEL) 
						{
							if ( RANK == 0 ) cout << "	# VELINIT #: GEN ... generating velocites via box Muller" << endl;	
						}
					else 
						{
							if ( RANK == 0 ) cout << "	# VELINIT #: READ ... reading velocities from coordinate file" << endl;	
						}
				}	
					...
					if (LINE=="HOOVER")
						{
							...
							if ( RANK == 0 ) cout << "	# THRMOST #: HOOVER... a Hoover time of " << CONTROLS.FREQ_UPDATE_THERMOSTAT << " will be used." << endl;	
						}
					else if (LINE=="VELSCALE")
						{
							...
							cin >> LINE; cin.ignore();	
							CONTROLS.FREQ_UPDATE_THERMOSTAT = double(atof(LINE.data()));
							if ( RANK == 0 ) cout << "	# THRMOST #: VELSCALE... Velocities will be scaled every " << CONTROLS.FREQ_UPDATE_THERMOSTAT << " MD steps." << endl;	
						}
					else
						{
					...
					if (LINE=="ANALYTICAL")
						{
							...
							if ( RANK == 0 ) cout << "	# PRESSUR #: ANALYTICAL" << endl;	
						}
					else if (LINE=="NUMERICAL")
						{
							CONTROLS.USE_NUMERICAL_PRESS = true;
							if ( RANK == 0 ) cout << "	# PRESSUR #: NUMERICAL" << endl;	
						}
					else
						{
			...
			// "Output control"
			...
			else if(LINE.find("# WRPCRDS #") != string::npos)
				{
					...
					if (LINE=="true"  || LINE=="True"  || LINE=="TRUE"  || LINE == "T" || LINE == "t")
						{
							CONTROLS.WRAP_COORDS = true;
							if ( RANK == 0 ) cout << "	# WRPCRDS #: true" << endl;	
						}
					else
						{
							CONTROLS.WRAP_COORDS = false;
							if ( RANK == 0 ) cout << "	# WRPCRDS #: false" << endl;							
						}
				}			
			...
			else if(LINE.find("# FRQDFTB #") != string::npos)
				{
					...
					if ( RANK == 0 ) cout << "	# FRQDFTB #: " << CONTROLS.FREQ_DFTB_GEN << endl;	
			
					if (CONTROLS.DELTA_T_FS > 0 && CONTROLS.N_MD_STEPS > 0)
						{
							if ( RANK == 0 ) cout << "		... printing every " << CONTROLS.FREQ_DFTB_GEN*CONTROLS.DELTA_T_FS << " fs, " << endl;
							if ( RANK == 0 ) cout << "		... printing " << CONTROLS.N_MD_STEPS/CONTROLS.FREQ_DFTB_GEN << " frames. " << endl; 
						}
				}
			...
			else if(LINE.find("# FRQENER #") != string::npos)
				{
					...
					if ( RANK == 0 ) cout << "	# FRQENER #: " << CONTROLS.FREQ_ENER << endl;	
				}		
					...
					if (LINE=="true"  || LINE=="True"  || LINE=="TRUE"  || LINE == "T" || LINE == "t")
						{
							...
							if ( RANK == 0 ) cout << "	# PRNTFRC #: true ...and will be printed every " << CONTROLS.FREQ_FORCE << " frames."<< endl;	
					...
					else if (LINE=="false" || LINE=="False" || LINE=="FALSE" || LINE == "F" || LINE == "f")
						{
							...
							if ( RANK == 0 ) cout << "	# PRNTFRC #: false" << endl;	
						}
					else
						{
...
{
#ifdef USE_MPI
	double *accel_sum = new double [3 * atoms] ;
	...
	MPI_Reduce(accel, accel_sum, 3 * atoms, MPI_DOUBLE, MPI_SUM, 0,
				  MPI_COMM_WORLD) ;
	...
	MPI_Reduce(&pot_energy, &petot, 1, MPI_DOUBLE, MPI_SUM, 0,
				  MPI_COMM_WORLD) ;
	...
	MPI_Reduce(&press, &pressure_tot, 1, MPI_DOUBLE, MPI_SUM, 0,
				  MPI_COMM_WORLD) ;
