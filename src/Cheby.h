...
public:
	...
	// Type of fix to use when inside inner cutoff.
	Cheby_fix cheby_fix_type ;
	
	// Distance for smoothing if inside inner cutoff.
	double inner_smooth_distance ;
	...
	Cheby(JOB_CONTROL &controls, FRAME &system, NEIGHBORS &neighbor_list, vector<PAIRS> &ff_2body, vector<int> &int_pair_map ) : CONTROLS(controls), SYSTEM(system), NEIGHBOR_LIST(neighbor_list), FF_2BODY(ff_2body), INT_PAIR_MAP(int_pair_map)
	{
		...
		cheby_fix_type = controls.cheby_fix_type ;
		inner_smooth_distance = controls.cheby_smooth_distance ;
	...
	void Print_3B(CLUSTER_LIST &TRIPS, string & ATM_TYP_1, string & ATM_TYP_2, string & ATM_TYP_3, int ij, int ik, int jk, int scan);
