...
static void optimal_ewald_params(double accuracy, int nat, double &alpha, double & rc, 
											int & kc, double & r_acc, double & k_acc, const double boxdim[3]) ;
...
// Calculate Ewald interactions.
{
  ...
  static double last_latcons[3] = {0.0, 0.0, 0.0} ;
    ...
    optimal_ewald_params(accuracy, nat, alpha,
								 r_cut, k_cut, r_acc, k_acc, Latcons) ;
    alphasq = alpha * alpha ;
								  ...
								  int & kc, double & r_acc, double & k_acc, const double boxdim[3])
{
  ...
  if ( boxdim[1] < min_boxdim ) 
	  min_boxdim = boxdim[1] ;
  if ( boxdim[2] < min_boxdim ) 
	  min_boxdim = boxdim[2] ;
...
// Calculate derivatives of the force wrt the coulomb parameters.
{
		  ...
		  Latcons[2] != last_latcons[2] ) 
	{
		...
		optimal_ewald_params(accuracy, nat, alpha, 
									r_cut, kmax,r_acc, k_acc, Latcons) ;
		alphasq = alpha * alpha ;
		printf("Ewald_Deriv:\n");
		printf("R-Space Ewald cutoff      = %13.6e\n", r_cut) ;
		printf("R-Space accuracy estimate = %13.6e\n", r_acc) ;
