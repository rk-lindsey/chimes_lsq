...
{  
	...
	// OLD WAY: XY, YZ, and XZ need to be included at a future date ...
	/*
	...
	for(int a1=a1start;a1 <= a1end; a1++)		// Double sum over atom pairs -- MPI'd over SYSTEM.ATOMS (prev-1) 
	{
		...
		{	
			...
			{			
				...
				SYSTEM.PRESSURE_TENSORS_XYZ_ALL[0].X -= S_r * RAB.X * RAB.X / rlen;
				SYSTEM.PRESSURE_TENSORS_XYZ_ALL[1].Y -= S_r * RAB.Y * RAB.Y / rlen;
				SYSTEM.PRESSURE_TENSORS_XYZ_ALL[2].Z -= S_r * RAB.Z * RAB.Z / rlen;
	...
	for(int ai=aistart;ai <= aiend; ai++)		// Double sum over atom pairs -- MPI'd over SYSTEM.ATOMS
	{
		...
		{	
			...
			if((FF_2BODY[curr_pair_type_idx].USE_OVRPRMS && SYSTEM.ATOMTYPE[ai] == FF_2BODY[curr_pair_type_idx].OVER_TO_ATM))
			{
				...
				SYSTEM.PRESSURE_TENSORS_XYZ_ALL[0].X -= tempr * (RVEC.X*RVEC.X)/rik;
				SYSTEM.PRESSURE_TENSORS_XYZ_ALL[1].Y -= tempr * (RVEC.Y*RVEC.Y)/rik;
				SYSTEM.PRESSURE_TENSORS_XYZ_ALL[2].Z -= tempr * (RVEC.Z*RVEC.Z)/rik;				
			}
		}
	}
