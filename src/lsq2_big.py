...
def main():
    ...
    if algorithm == 'svd':
        ...
        x,y = fit_dlars(num_nodes, num_cores, alpha_val, split_files, algorithm, read_output, weights_file, normalize, Afile, bfile, is_active,restart_dlasso_dlars)
        np = count_nonzero_vars(x)
        nvars = np
...
def fit_dlars(num_nodes, num_cores, alpha_val, split_files, algorithm, read_output, weights_file, normalize, Afile, bfile, is_active, restart_dlasso_dlars):
    ...
    if not read_output:
        ...
        path=os.path.dirname(os.path.abspath(__file__))
        dlars_file = path[:-3] + "contrib/dlars/src/dlars_A.xxxxx.txt"
	...
	#dlars_file = "/my/hardcoded/path/"
        ...
        if os.path.exists(dlars_file):
            ...
            if  is_active == True: # Then we're using the parallel driver and files are named differently

		command = exepath + " " + Afile + " " + bfile + " dim.txt --lambda=" + `alpha_val`
		normalize = True
		
		print "here-1",normalize,is_active
