...
class container_periodic_base : public unitcell, public voro_base {
	public:
		...
		inline bool initialize_voronoicell(voronoicell_neighbor &c,int ijk,int q,int ci,int cj,int ck,int &i,int &j,int &k,double &x,double &y,double &z,int &disp) {
			(void)ci; (void)cj; (void)ck; (void)disp; /* Suppress "unused parameter" warning */
			c=unit_voro;
		...
		inline void initialize_search(int ci,int cj,int ck,int ijk,int &i,int &j,int &k,int &disp) {
			(void)ci; (void)cj; (void)ck; (void)disp; (void)ijk; /* Suppress "unused parameter" warning */
			i=nx;j=ey;k=ez;
		}
		...
		inline int region_index(int ci,int cj,int ck,int ei,int ej,int ek,double &qx,double &qy,double &qz,int &disp) {
			(void)qy; (void)qz; (void)disp; /* Suppress "unused parameter" warning */
...
class container_periodic_poly : public container_periodic_base, public radius_poly {
	public:
		...
		void draw_cells_gnuplot(c_loop &vl,FILE *fp) {
			voronoicell_neighbor c;double *pp;
...
/*			} else {
		...
		template<class c_loop>
		inline bool compute_cell(voronoicell_neighbor &c,c_loop &vl) {
		 ...
		 * computed because it was removed entirely for some reason,
		 * then the routine returns false. */
		inline bool compute_cell(voronoicell_neighbor &c,int ijk,int q) {
