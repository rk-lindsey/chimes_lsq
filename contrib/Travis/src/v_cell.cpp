...
void voronoicell_base::add_memory(voronoicell_neighbor &vc,int i,int *stackp2) {
	int s=(i<<1)+1;
	if(mem[i]==0) {
...
void voronoicell_base::add_memory_vertices(voronoicell_neighbor &vc) {
...
void voronoicell_base::add_memory_vorder(voronoicell_neighbor &vc) {
...
void voronoicell_base::check_relations() {
	...
	for(i=0;i<p;i++) for(j=0;j<nu[i];j++) if(ed[ed[i][j]][ed[i][nu[i]+j]]!=i)
		mprintf("Relational error at point %d, edge %d.\n",i,j);
}
...
void voronoicell_base::check_duplicates() {
	...
	for(i=0;i<p;i++) for(j=1;j<nu[i];j++) for(k=0;k<j;k++) if(ed[i][j]==ed[i][k])
		mprintf("Duplicate edges: (%d,%d) and (%d,%d) [%d]\n",i,j,i,k,ed[i][j]);
}
 ...
 * \param[in,out] up */
inline bool voronoicell_base::search_for_outside_edge(voronoicell_neighbor &vc,int &up) {
...
bool voronoicell_base::nplane(voronoicell_neighbor &vc,double x,double y,double z,double rsq,int p_id) {
	...
	try {
		if(uw==1) {
			...
			if(us==nu[up]) {
				if (!g_bVoroSilent)
					mprintf("\nvoronoicell_base::nplane(): Error B1.");
				return false;
			}
			...
			while(lw==1) {
				...
				if(us==ls) {
					...
					if(us==nu[up]) {
						if (!g_bVoroSilent)
							mprintf("\nvoronoicell_base::nplane(): Error B2.");
						return false;
					}
				}
				ls=ed[up][nu[up]+us];
		...
		// the plane.
#if VOROPP_VERBOSE >=1
		eprintf("voro++: Bailed out of convex calculation\n");
#endif
		qw=1;lw=0;
		for(qp=0;qp<p;qp++) {
			qw=m_test(qp,q);
		...
		if (qp==p)
		{
			...
			{
				if (!g_bVoroSilent)
					mprintf("\nvoronoicell_base::nplane(): Error B3.");
				return false;
			}
		}
	}
	...
	if(complicated_setup) {
		...
		{
			if (!g_bVoroSilent)
				mprintf("\nvoronoicell_base::nplane(): Error B4.");
			return false;
		}
		...
		if(lw!=-1) {
			...
			do {
				...
				{
					if (!g_bVoroSilent)
						mprintf("\nvoronoicell_base::nplane(): Error B5.");
					return false;
				}
				lp=ed[up][i];
				lw=m_test(lp,l);
 ...
 *         was successful. */
inline bool voronoicell_base::collapse_order2(voronoicell_neighbor &vc) {
 ...
 *         successful. */
inline bool voronoicell_base::collapse_order1(voronoicell_neighbor &vc) {
	int i,j,k;
	while(mec[1]>0) {
		up=0;
#if VOROPP_VERBOSE >=1
...
inline bool voronoicell_base::delete_connection(voronoicell_neighbor &vc,int j,int k,bool hand) {
...
void voronoicell_base::print_edges() {
	...
	for(int i=0;i<p;i++,ptsp+=3) {
		mprintf("%d %d  ",i,nu[i]);
		for(j=0;j<nu[i];j++) mprintf(" %d",ed[i][j]);
		mprintf("  ");
		while(j<(nu[i]<<1)) mprintf(" %d",ed[i][j]);
		mprintf("   %d",ed[i][j]);
		print_edges_neighbors(i);
		mprintf("  %g %g %g %p",*ptsp,ptsp[1],ptsp[2],(void*) ed[i]);
...
void voronoicell_neighbor::print_edges_neighbors(int i) {
	if(nu[i]>0) {
		int j=0;
		mprintf("     (");
		while(j<nu[i]-1) mprintf("%d,",ne[i][j++]);
		mprintf("%d)",ne[i][j]);
	} else mprintf("     ()");
}
