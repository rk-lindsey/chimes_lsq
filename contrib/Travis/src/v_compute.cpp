...
voro_compute::voro_compute(container_periodic_poly &con_,int hx_,int hy_,int hz_) :
...
inline void voro_compute::scan_all(int ijk,double x,double y,double z,int di,int dj,int dk,particle_record &w,double &mrs) {
...
void voro_compute::find_voronoi_cell(double x,double y,double z,int ci,int cj,int ck,int ijk,particle_record &w,double &mrs) {
...
inline void voro_compute::add_to_mask(int ei,int ej,int ek,int *&qu_e) {
 ...
 * \param[in] (ei,ej,ek) the block to consider.
 * \param[in,out] qu_e a pointer to the end of the queue. */
inline void voro_compute::scan_bits_mask_add(unsigned int q,unsigned int *mijk,int ei,int ej,int ek,int *&qu_e) {
...
bool voro_compute::compute_cell(voronoicell_neighbor &c,int ijk,int s,int ci,int cj,int ck) {
	...
	{
		if (!g_bVoroSilent)
			mprintf("\nvoro_compute::compute_cell(): Error A1.");
		return false;
	}
	con.r_init(ijk,s);
	...
	for(l=0;l<s;l++) {
		...
		{
			if (!g_bVoroSilent)
				mprintf("\nvoro_compute::compute_cell(): Error A2(x1=%f,y1=%f,z1=%f,rs=%f,l=%d,s=%d,ijk=%d).",x1,y1,z1,rs,l,s,ijk);
			return false;
		}
	}
	l++;
	while(l<co[ijk]) {
		...
		{
			if (!g_bVoroSilent)
				mprintf("\nvoro_compute::compute_cell(): Error A3(x1=%f,y1=%f,z1=%f,rs=%f,l=%d,s=%d,ijk=%d).",x1,y1,z1,rs,l,s,ijk);
			return false;
		}
		l++;
	}
		...
		if(co[ijk]>0) {
			...
			if(!con.r_ctest(crs,mrs)) {
				do {
					...
					{
						if (!g_bVoroSilent)
							mprintf("\nvoro_compute::compute_cell(): Error A4.");
						return false;
					}
					l++;
				} while (l<co[ijk]);
				...
				do {
					...
					{
						if (!g_bVoroSilent)
							mprintf("\nvoro_compute::compute_cell(): Error A5.");
						return false;
					}
					l++;
				} while (l<co[ijk]);
		...
		if(co[ijk]>0) {
			...
			if(!con.r_ctest(crs,mrs)) {
				do {
					...
					{
						if (!g_bVoroSilent)
							mprintf("\nvoro_compute::compute_cell(): Error A6.");
						return false;
					}
					l++;
				} while (l<co[ijk]);
				...
				do {
					...
					{
						if (!g_bVoroSilent)
							mprintf("\nvoro_compute::compute_cell(): Error A7.");
						return false;
					}
					l++;
				} while (l<co[ijk]);
	...
	while(qu_s!=qu_e) {
		...
		if(co[ijk]>0) {
			...
			do {
				...
				{
					if (!g_bVoroSilent)
						mprintf("\nvoro_compute::compute_cell(): Error A8.");
					return false;
				}
				l++;
			} while (l<co[ijk]);
...
bool voro_compute::corner_test(voronoicell_neighbor &c,double xl,double yl,double zl,double xh,double yh,double zh) {
 ...
 * \return False if the block may intersect, true if does not. */
inline bool voro_compute::edge_x_test(voronoicell_neighbor &c,double x0,double yl,double zl,double x1,double yh,double zh) {
 ...
 *                    block furthest away from the cell center.
 * \return False if the block may intersect, true if does not. */
inline bool voro_compute::edge_y_test(voronoicell_neighbor &c,double xl,double y0,double zl,double xh,double y1,double zh) {
 ...
 *                    block furthest away from the cell center.
 * \return False if the block may intersect, true if does not. */
inline bool voro_compute::edge_z_test(voronoicell_neighbor &c,double xl,double yl,double z0,double xh,double yh,double z1) {
 ...
 *                    block.
 * \return False if the block may intersect, true if does not. */
inline bool voro_compute::face_x_test(voronoicell_neighbor &c,double xl,double y0,double z0,double y1,double z1) {
	con.r_prime(xl*xl);
 ...
 *                    block.
 * \return False if the block may intersect, true if does not. */
inline bool voro_compute::face_y_test(voronoicell_neighbor &c,double x0,double yl,double z0,double x1,double z1) {
	con.r_prime(yl*yl);
 ...
 *                    block.
 * \return False if the block may intersect, true if does not. */
inline bool voro_compute::face_z_test(voronoicell_neighbor &c,double x0,double y0,double zl,double x1,double y1) {
	con.r_prime(zl*zl);
...
inline void voro_compute::add_list_memory(int*& qu_s,int*& qu_e) {
	qu_size<<=1;
