program gr_calc
  ...
  !
  implicit none
  integer, parameter :: MAXATOM = 4000                 ! Max number of atoms.
  integer, parameter :: MAXHISTO = 4000                ! Maximum length of the histogram.
  ...
  if ( iformat .ne. 4 ) then
     ...
     if ( natom > MAXATOM ) then
        write(*,*) 'Too many atoms: increase MAXATOM'
        stop
     endif
  end if
  read(*,*) itype1, itype2
...
89      format(e13.5, ' ' , e13.5)
     ...
     if ( nconfig .gt. 0 ) then
        ...
        open(4,file='rdf.dat')	! WRITE THE RDF TO A FILE
	...
	do i = 1, icut
  	   write(4, *)  rbin(i), gr(i)
           write(*,99) rbin(i), gr(i)
99         format(e13.5, ',' , e13.5)
        end do
	close(4)	! WRITE THE RDF TO A FILE -- CLOSE THE FILE
