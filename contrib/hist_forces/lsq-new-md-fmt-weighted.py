...
if(len (sys.argv) == 7): 	
	...
	weight_file_weights=open(sys.argv[6],"r").readlines()
...
#################################
# Use b matrix to generate weights
#################################

# Note: We want entries with smaller forces
#       to make a greater contrib. to the fit

weights = zeros((nlines))


## THE SCENARIOS: 
#
# Originally: "low lying" (i.e. |small|) forces over-predicted
#
# Inverse weights: Original problem exaggerated


for i in range(0,nlines):  
#	if (b[i] != 0):
#		weights[i] = sqrt(abs(1/b[i]))   # Weighting scheme 1: inverse relationship 
#		weights[i] = exp(-1*abs(1/b[i])) # Weighting scheme 2: exponential relationship 
#		weights[i] = exp(-1*abs(1/b[i])/2000) # Weighting scheme 4: exponential relationship w/ weighting temperature
#		weights[i] = exp(-1*abs(b[i])) # Weighting scheme 4: exponential relationship no inverse
#		weights[i] = sqrt(1/2.0/3.14/WIDTH/WIDTH) * exp( -1.0*pow((abs(b[i]) - MID),2)/2/WIDTH/WIDTH) * MAGNITUDE
#		weights[i] = exp(-1*abs(b[i])/10000) # Weighting scheme 4: exponential relationship no inverse, add T_W		
#	else:
#		weights[i] = 1 
	if(len (sys.argv) == 7):
		weights[i] = weight_file_weights[i]
	else:
		weights[i] = 1
...
if(len (sys.argv) == 7):
	...
	U,D,VT=numpy.linalg.svd(A)	# Non-weighted version
	
if(len (sys.argv) == 7):	
	Dmat=array((transpose(weightedA)))
else:
	Dmat=array((transpose(A)))	# Non-weighted version
...
y=dot(A,x)				# <<--------- A may need weights applied...
Z=0.0
